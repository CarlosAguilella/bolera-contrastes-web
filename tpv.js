(function () {
  ...
  function renderFloor() {
    ...
    return `${topbar("Sala", `${opened} mesas abiertas · toca una mesa para tomar nota`)}<section class="tpv-floor-panel"><div class="tpv-panel__head"><div><h2>Plano del local</h2><span>Verde: lista · Ámbar: comanda abierta</span></div><button class="tpv-action is-secondary" type="button" data-open-table="50">Barra rápida</button></div><div class="tpv-floor" aria-label="Plano de mesas"><div class="tpv-floor__bar"><span>Barra</span><i></i><i></i><i></i><i></i></div><div class="tpv-floor__plants" aria-hidden="true">●<br>●<br>●<br>●</div>${Core.tables.map((table) => { const ticket = state.data.tables[table.id]; const status = tableStatus(table.id); return `<button type="button" class="tpv-floor-table is-${status} ${table.area === "wall" ? "is-wall" : ""}" style="--x:${table.x};--y:${table.y}" data-open-table="${table.id}" aria-label="${table.name}, ${statusText(status)}"><span class="tpv-floor-table__desk"></span><strong>${table.id}</strong><small>${ticket ? Core.formatEuros(total(ticket.lines)) : statusText(status)}</small></button>`; }).join("")}</div><div class="tpv-floor-legend"><span><i class="is-free"></i>Libre</span><span><i class="is-open"></i>Abierta</span><span><i class="is-preparing"></i>En cocina</span><span><i class="is-ready"></i>Lista para servir</span></div></section>`;
  }
