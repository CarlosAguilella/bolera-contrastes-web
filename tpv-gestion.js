(function () {
  ...
  function renderAccounting() {
    ...
    return `<section class="tpv-gestion-content"><div class="tpv-gestion-metrics"><article><span>Facturas registradas</span><strong>${invoices.length}</strong></article><article><span>Compras con IVA</span><strong>${Core.formatEuros(total)}</strong></article><article><span>IVA soportado</span><strong>${Core.formatEuros(vat)}</strong></article><article><span>Lectura IA</span><strong>Preparada</strong></article></div><section class="tpv-gestion-card"><header><div><h2>Registrar factura de proveedor</h2><span>Base de datos preparada para que la IA extraiga proveedor, productos, códigos, cantidades e IVA.</span></div></header><form class="tpv-invoice-form" data-invoice-form><label>Proveedor<input name="supplierName" required placeholder="Distribuciones Ejemplo"></label><label>Número factura<input name="invoiceNumber" placeholder="Opcional"></label><label>Fecha<input name="invoiceDate" type="date" value="${new Date().toISOString().slice(0, 10)}" required></label><label class="tpv-invoice-form__wide">Líneas de factura<textarea name="lines" required placeholder="Producto | código | cantidad | precio unitario sin IVA (€) | IVA (%)\nPan bocadillo | 843... | 24 | 0.42 | 10\nBacon lonchas | BAC-01 | 10 | 4.50 | 10"></textarea><small>Una línea por producto. Así dejaremos preparado el formato que comprobará la IA al leer una factura real.</small></label><label class="tpv-invoice-form__wide">Texto original u observaciones<textarea name="sourceText" placeholder="Pega aquí el texto extraído de una factura si lo tienes."></textarea></label><button class="tpv-action" type="submit">Guardar factura</button></form></section><section class="tpv-gestion-card"><header><div><h2>Facturas y gastos de compra</h2><span>Las futuras lecturas con IA quedarán como “pendientes de revisar” antes de afectar al stock o al coste.</span></div></header><div class="tpv-invoice-list">${invoices.length ? invoices.map((invoice) => `<article><header><div><b>${escapeHtml(invoice.supplier_name)}</b><small>${escapeHtml(invoice.invoice_number || "Sin número")} · ${new Date(`${invoice.invoice_date}T12:00:00`).toLocaleDateString("es-ES")}</small></div><strong>${Core.formatEuros(invoice.total_cents)}</strong></header><ul>${(invoice.supplier_invoice_lines || []).map((line) => `<li><span>${escapeHtml(line.product_name)}${line.product_code ? ` · ${escapeHtml(line.product_code)}` : ""}</span><span>${line.quantity} ${escapeHtml(line.unit || "uds.")}</span><b>${Core.formatEuros(line.line_total_cents)}</b></li>`).join("")}</ul><footer><span>Base: ${Core.formatEuros(invoice.subtotal_cents)} · IVA: ${Core.formatEuros(invoice.vat_cents)}</span><em>${invoice.classification_status === "manual" ? "Registrada manualmente" : "Pendiente de revisión"}</em></footer></article>`).join("") : `<p class="tpv-gestion-empty">Aún no hay facturas. Cuando compartas una de ejemplo, conectaremos la lectura automática y la validación de sus campos.</p>`}</div></section><aside class="tpv-management-note"><h2>Cómo llegará la IA</h2><p>La factura se subirá como PDF o foto. La IA propondrá proveedor, fecha, productos, códigos, cantidades, precios e IVA.</p><p>Antes de guardarla, se mostrará una revisión para evitar errores y relacionar cada línea con la materia prima correcta.</p></aside></section>`;
  }
  function priceModal() {
