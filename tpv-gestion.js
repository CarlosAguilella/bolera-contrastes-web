(function () {
  ...
  root.addEventListener("submit", (event) => {
    if (event.target.matches("[data-invoice-upload-form]")) {
      event.preventDefault();
      const file = event.target.querySelector('input[name="invoiceFile"]')?.files?.[0];
