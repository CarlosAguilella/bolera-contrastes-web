(function () {
  ...
  function renderArticles() {
    ...
    return `<section class="tpv-gestion-content">${categoryOrder}<section class="tpv-gestion-card"><header><div><h2>Catálogo del restaurante</h2><span>${products.length} artículos · ${photoCount} con foto real · sincronizados en todos los TPV</span></div><div class="tpv-gestion-header-actions"><label class="tpv-search"><span>⌕</span><input type="search" value="${escapeHtml(state.search)}" placeholder="Buscar artículo" data-search-products></label>${canManage ? `<a class="tpv-action" href="tpv-articulo-nuevo">Nuevo artículo</a>` : ""}</div></header><div class="tpv-articles-table"><div class="tpv-articles-row is-heading"><span>Artículo</span><span>Familia</span><span>PVP</span><span>Coste compra</span><span>Margen</span><span></span></div>${catalogRows}</div></section><aside class="tpv-management-note"><h2>Fotos reales de la carta</h2><p>Entra en <strong>Editar</strong> y toma o selecciona la foto del producto real. No se usan imágenes genéricas.</p><p>Las fotos se guardan en la base central y se ven en todos los TPV conectados.</p></aside></section>`;
  }
